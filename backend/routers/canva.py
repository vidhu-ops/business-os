...
def _admin_setup_html(*, key: str, authorize_url: str, redirect_uri: str, status: dict) -> str:
...
<head>
  ...
  <style>
    ...
    .warn {{ border-color: #c90; background: #fffbe6; }}
...
<body>
  ...
  <p class="muted">Status: <strong id="st">{status.get("status", "")}</strong> &middot; ready: <strong>{ready}</strong></p>

  <div class="card warn">
    <h2>Redirect URI (must match Canva Developer Portal exactly)</h2>
    <p class="muted">In Canva &rarr; your app &rarr; Authentication &rarr; Authorized redirects, add this URL <strong>exactly</strong> (no trailing slash):</p>
    <textarea id="redirectUri" rows="2" readonly>{redirect_uri}</textarea>
    <p><button type="button" onclick="navigator.clipboard.writeText(document.getElementById('redirectUri').value)">Copy redirect URI</button></p>
  </div>
...
def admin_canva_setup(key: str = "") -> HTMLResponse:
    ...
    return HTMLResponse(_admin_setup_html(key=key, authorize_url=authorize_url, redirect_uri=canva_redirect_uri(), status=status))
...
def admin_canva_authorize_url(
    ...
    return {
        ...
        "instructions": "Register redirect_uri in Canva Developer Portal, then open authorize_url on your phone.",
